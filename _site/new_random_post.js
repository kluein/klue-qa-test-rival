...
function generateContent(date, randomName) {
    const title = faker.random.words(6); // generates 6 random English-like words
    const subtitle = faker.random.words(8); // generates 8 random English-like words
...
`;
  ...
  for (let i = 0; i < 5; i++) {
    let paragraph = '';
    // generate 4 sentences per paragraph
    for (let j = 0; j < 4; j++) {
      const sentence = faker.random.words(6); // generate a sentence of 6 words
      paragraph += `${sentence}. `;
    }
